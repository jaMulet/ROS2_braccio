...
/**
 * This functions allow you to control all the servo motors (ORIGINAL VERSION)
 * 
 ...
 */
int _Braccio::ServoMovement_ini(int stepDelay, int vBase, int vShoulder, int vElbow,int vWrist_ver, int vWrist_rot, int vgripper) {
 ...
 * @param vgripper next gripper servo motor degree
 */
int _Braccio::ServoMovement(int vBase, int vShoulder, int vElbow,int vWrist_ver, int vWrist_rot, int vgripper) {
  ...
  //Perform movement for each servo motor		
  base.write(vBase);
  shoulder.write(vShoulder);
  elbow.write(vElbow);
  wrist_rot.write(vWrist_ver);
  wrist_ver.write(vWrist_rot);
